...
Many analytic tasks can be completed with `mutate`, `filter`, `select`, `group_by`, `summarize` and `arrange`. But sometimes you'll need an expanded tool kit. Let's look at a few more of the most useful functions.
...
Sometimes we need to use data available from multiple sources. To take advantage of the `dplyr` functions we used previously, we need data points of interest to be in the same dataframe. The join family of functions helps here.
...
```{r joining}
# left_join adds all the data from the right df to the matching rows of the left dataframe
...
```{r join_venn_diagram}
knitr::include_graphics("img/join-venn.png", dpi = 400)
```
...
```{r join_venn_diagram}
knitr::include_graphics("img/tidyr-fig1.png", dpi = 400)
```
...
The function `pivot_longer()` will turn a wide dataframe into a longer one by transformin the observation variables into a single variable. This is sometimes called "melting" your data, because it melts the table from wide to long. Those data will be melted into two variables: one for the variable names, and the other for the variable values. Often, the terms "key" and "value" are used to refer to the variable names and variable values.

```{r}
gap_long <- gap_wide %>%
  pivot_longer(-c(country, continent), names_to = "obstype_year", "obs_value")
head(gap_long)
...
We are nearly where we want to be. Let's do a little more clean up to make "obstype_year" more usable.

```{r}
# Approach 1
gap_long_tidy1 <- gap_long %>%
  mutate(obstype = gsub("_.*", "", obstype_year),
...
Regular expressions: for handling strings.

```{r}
# Approach 2
gap_long_tidy2 <- gap_long %>%
  tidyr::separate(obstype_year, into = c("obstype", "year"), sep = "_") %>%
...
`pivot_wider` is the opposite of pivot_longer. It makes new columns using values in one column as the new column names, and the values in a second column as the new column values.

```{r}
gap_long_tidy1 %>%
  pivot_wider(names_from = obstype, values_from = value) %>%
  head()
...
1. Starting with the gap data set, use `tidyr::pivot_wider` to create a data frame of population over time, where each row is a country, and each column is a year.
...
```{r}
diagnosis <- data.frame(pt = c("M.H.", "P.J.", "K.G.", "S.C."),
                       date_of_diagnosis = c("2020-08-01", "2019-01-11", "2020-03-11", "2020-04-18"),
                       stringsAsFactors = FALSE)
dob <- data.frame(pt = c("M.H.", "P.J.", "K.G.", "S.C."),
                  date_of_birth = c("1990-11-10", "1972-11-14", "2002-10-31", "1954-08-22"),
                  stringsAsFactors = FALSE)

diag <- diagnosis %>%
  mutate(date_of_diagnosis = as.Date(date_of_diagnosis, format = "%Y-%m-%d"))

dob <- dob %>%
  mutate(dob = as.Date(date_of_birth, format = "%Y-%m-%d"))

left_join(dob, diag) %>%
  mutate(age = as.numeric(date_of_diagnosis - dob)/365.25)
