...
So far we've been working with character data, numeric or integer data, and logicals. There's a few other more complicated data types you'll often come across in health/pharmaceutical data: factors and dates. Sometimes, you'll also find you're misising a type of data altogether! Let's take a little break from `dplyr` to learn more about these types of data.
...
* Useful for statistics (ex: modeling GDP per capita as a function of time and continent)
...
gap2 <- read.csv(file.path('data', 'gapminder-FiveYearData.csv'), stringsAsFactors = TRUE)

class(gap2$continent)
head(gap2$continent) # What order are the factors in?
levels(gap2$continent) 

summary(select(gap2, continent))
summary(select(gap, continent))

head(gap2)

head(as.character(gap2$continent))
```
...
R is generally pretty helpful with comparing factors:
```{r fac_logicals}
pre_factored <- c("cat", "dog", "cat")
fact1 <- factor(pre_factored, levels = c("cat", "dog"))
fact2 <- factor(pre_factored, levels = c("dog", "cat"))

as.numeric(fact1)
as.numeric(fact2)

levels(fact1) == levels(fact2)
fact1 == fact2
...
```{r} 
gap2 %>%
  mutate(pop = ifelse(continent == "Oceania", 0, pop)) %>%
  group_by(continent, .drop = FALSE) %>%
...
patients <- data.frame(ID = c(rep(1,3), rep(2,2)),
...
patients %>%
  group_by(ID) %>%
  mutate(dt = difftime(datetime, min(datetime), units = 'hours')) %>%
...
filter(gap, lifeExp > lower & lifeExp < upper)
```
...
fact1 <- factor(chars, levels = chars)
fact1
...
date1 <- as.Date(date1, format = "%b %d %Y")
date2 <- as.Date(date2, format = "%Y-%m-%d")

question_prompt <- as.Date("1994-03-08", format = "%Y-%m-%d")

date1 - question_prompt
date2 - question_prompt
...
identical(test_data_frame, select(mtcars, -mpg))
